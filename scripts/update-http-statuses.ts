// All codes from this file are from the below repository.
...
async function run() {
  console.log("Updating src/stoker/http-status-codes.ts and src/stoker/http-status-phrases.ts");
  ...
  const statusCodeFile = project.createSourceFile("src/stoker/http-status-codes.ts", {}, {
    overwrite: true,
  });
  ...
  const phrasesFile = project.createSourceFile("src/stoker/http-status-phrases.ts", {}, {
    overwrite: true,
  });
  ...
  await project.save();
  await execSync("npx eslint --fix ./src/stoker/http-status-codes.ts ./src/stoker/http-status-phrases.ts");
  console.log("Successfully generated src/stoker/http-status-codes.ts and src/stoker/http-status-phrases.ts");
}
